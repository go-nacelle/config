...
// 2020-04-01T14:24:07-05:00
// using the command
// $ go-mockgen -f github.com/go-nacelle/config -i Sourcer -o sourcer.go
...
func NewMockSourcer() *MockSourcer {
	return &MockSourcer{
		...
		DumpFunc: &SourcerDumpFunc{
			defaultHook: func() (map[string]string, error) {
				return nil, nil
			},
		},
		GetFunc: &SourcerGetFunc{
			defaultHook: func([]string) (string, config.SourcerFlag, error) {
				return "", 0, nil
			},
		},
		TagsFunc: &SourcerTagsFunc{
...
type SourcerDumpFunc struct {
	defaultHook func() (map[string]string, error)
	hooks       []func() (map[string]string, error)
...
func (m *MockSourcer) Dump() (map[string]string, error) {
	r0, r1 := m.DumpFunc.nextHook()()
	m.DumpFunc.appendCall(SourcerDumpFuncCall{r0, r1})
	return r0, r1
}

// SetDefaultHook sets function that is called when the Dump method of the
// parent MockSourcer instance is invoked and the hook queue is empty.
func (f *SourcerDumpFunc) SetDefaultHook(hook func() (map[string]string, error)) {
	f.defaultHook = hook
}
...
// front of the queue and discards it. After the queue is empty, the default
// hook function is invoked for any future action.
func (f *SourcerDumpFunc) PushHook(hook func() (map[string]string, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
// SetDefaultReturn calls SetDefaultDefaultHook with a function that returns
// the given values.
func (f *SourcerDumpFunc) SetDefaultReturn(r0 map[string]string, r1 error) {
	f.SetDefaultHook(func() (map[string]string, error) {
		return r0, r1
	})
}
...
// PushReturn calls PushDefaultHook with a function that returns the given
// values.
func (f *SourcerDumpFunc) PushReturn(r0 map[string]string, r1 error) {
	f.PushHook(func() (map[string]string, error) {
		return r0, r1
	})
}

func (f *SourcerDumpFunc) nextHook() func() (map[string]string, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
...
type SourcerGetFunc struct {
	defaultHook func([]string) (string, config.SourcerFlag, error)
	hooks       []func([]string) (string, config.SourcerFlag, error)
...
func (f *SourcerGetFunc) SetDefaultReturn(r0 string, r1 config.SourcerFlag, r2 error) {
	f.SetDefaultHook(func([]string) (string, config.SourcerFlag, error) {
		return r0, r1, r2
	})
}
...
func (f *SourcerGetFunc) PushReturn(r0 string, r1 config.SourcerFlag, r2 error) {
	f.PushHook(func([]string) (string, config.SourcerFlag, error) {
		return r0, r1, r2
	})
}

func (f *SourcerGetFunc) nextHook() func([]string) (string, config.SourcerFlag, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
