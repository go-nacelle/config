...
func (m *MockConfig) Dump() (map[string]string, error) {
	...
	m.DumpFunc.appendCall(ConfigDumpFuncCall{r0, r1})
	return r0, r1
}

// SetDefaultHook sets function that is called when the Dump method of the
// parent MockConfig instance is invoked and the hook queue is empty.
func (f *ConfigDumpFunc) SetDefaultHook(hook func() (map[string]string, error)) {
	f.defaultHook = hook
}
...
// front of the queue and discards it. After the queue is empty, the default
// hook function is invoked for any future action.
func (f *ConfigDumpFunc) PushHook(hook func() (map[string]string, error)) {
	f.mutex.Lock()
	f.hooks = append(f.hooks, hook)
	f.mutex.Unlock()
}
...
// SetDefaultReturn calls SetDefaultDefaultHook with a function that returns
// the given values.
func (f *ConfigDumpFunc) SetDefaultReturn(r0 map[string]string, r1 error) {
...
// PushReturn calls PushDefaultHook with a function that returns the given
// values.
func (f *ConfigDumpFunc) PushReturn(r0 map[string]string, r1 error) {
	f.PushHook(func() (map[string]string, error) {
		return r0, r1
	})
}

func (f *ConfigDumpFunc) nextHook() func() (map[string]string, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()
